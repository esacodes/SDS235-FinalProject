...
```{r}
my_colors <- RColorBrewer::brewer.pal(6, "BuGn")[6:3]

new_data %>% 
  filter(Group == "By Education",
         Indicator == "Received Counseling or Therapy, Last 4 Weeks") %>% 
  ggplot(aes(x = total_cases, y = Value)) +
  #geom_ribbon(aes(x = total_cases, ymax = HighCI, ymin = LowCI, fill = Subgroup), alpha = .1, show.legend = FALSE) +
  geom_point(aes(color = Subgroup)) +
  labs(title = "Access to Counseling or Therapy Over the Pandemic", subtitle = "Given number of COVID patients") +
  theme(legend.position = "right") +
  ...
  theme_classic() +
  scale_x_continuous("COVID-19 Cases in the US During the Time Period") +
  scale_y_continuous("% of US Adults Who Received Counseling") 
```
References:
